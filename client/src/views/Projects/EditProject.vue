...
onMounted(() => { getDetailProject() });
const getDetailProject = async () => {
    await axios.get(`/projects/${id}/edit`).then(
        res => { 
...
const save = () => {
    sendRequest('PUT', form.value, `/projects/${id}`, '/projects');
    form.value.title = '';
...
<template>
    <form @submit.prevent="save" class="flex justify-around flex-col items-end space-x-4 w-[80%] mx-auto mt-8">
        ...
        <div class="w-full">
            <button type="submit" class="bg-blue-500 rounded-sm text-white px-4 py-2 w-full">Editar Proyecto</button>
        </div>
    </form>
</template>